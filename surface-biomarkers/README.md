...
- translational prioritization using surface/localization and antibody evidence;
- antibody panel design with positive, exclusion and backup markers;
...
Because the models are trained on single-nuclei RNA-seq, protein assays are reported as candidates, not as validated markers.

The package includes internal copies of:
...
Use `surface_db="internal"` and `protein_atlas="internal"` to use them. Pass `protein_atlas=None` if you want to skip antibody metadata.

```python
from surface_biomarkers import prioritize_signature_validation
...
```python
from surface_biomarkers import save_signature_validation_report
...
validation_priority = save_signature_validation_report(
    ...
    surface_db="internal",
    protein_atlas="internal",
    output_path="cluster_outputs/signature_validation_priority.csv",
    signature_only=True,
)
```
...
`design_antibody_panel()` recommends marker combinations from the translational validation table. It is meant to answer a different question from individual gene ranking:

```text
Which small marker combination should I test for this cluster and assay?
```
...
```python
from surface_biomarkers import design_antibody_panel
from surface_biomarkers.plots import plot_panel_benchmark

panel = design_antibody_panel(
...
plot_panel_benchmark(
    panel,
    "cluster_outputs/panel_benchmark_FACS.svg",
    cluster_names=cluster_names,
    show=True,
)
```

The benchmark plot compares:

```text
panel_score
panel_size and backup markers
redundancy_penalty
...
If you have a gene-gene redundancy/correlation matrix, pass it to penalize redundant markers:

```python
panel = design_antibody_panel(
    markers=validation_priority,
    application="CyTOF",
    redundancy_matrix=gene_correlation_matrix,
    redundancy_threshold=0.85,
)
```

Important: panel design uses `expression_shap_corr` to separate positive markers from negative/exclusion markers, so this column is kept in the validation report.
...
Prioritize signatures for translational validation:

```bash
surface-biomarkers validate-signatures cluster_outputs \
  --ranking gene_ranking_by_cluster.csv \
  --signatures signatures.json \
  --output signature_validation_priority.csv
```
...
src/surface_biomarkers/
  ...
  validation.py    # translational prioritization with surface/antibody evidence
  panel_design.py  # compact antibody panel design
  plots.py         # ROC, confusion matrix, SHAP, heatmaps, panels
  resources/
    db_surface.csv # internal surface-gene database
    proteinatlas.tsv # internal Human Protein Atlas antibody metadata
```
...
- `prioritize_signature_validation()` ranks individual genes; `design_antibody_panel()` recommends marker combinations.
